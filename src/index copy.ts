...
import { processString } from "./handlers/stringfunctions";
...
export function capture() {
	...
	console.log("dialog:", dialogBool);
	if(dialogBool){
		let [name, dialogText] = [dialog.readTitle(img), dialog.readDialog(img, dialog.checkDialog(img))].map(v => processString(String(v)));
		console.log("dialogText:", JSON.stringify(dialogText));
		output.insertAdjacentHTML("beforeend", `<div> ${dialogText} </div>`);
	}
	if (!window.alt1) {
