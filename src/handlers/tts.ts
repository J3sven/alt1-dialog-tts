...
export class AwsTextToSpeech {
    ...
    private lastProcessedString: string | null = null;
    private femaleVoice = "Joanna";
    private maleVoice = "Matthew";
    ...
    public async speak(text: string, name: string, player: any): Promise<void> {
        ...
        if (this.isPlaying && this.audioQueue.indexOf(text) !== -1) {
            return;
        }
        ...
        console.log('name', name, player.self, player.selfFemale, player);
...
export class MeSpeakTextToSpeech {
    private audioQueue: { text: string; voice: string }[] = [];
    private isPlaying = false;
    private lastProcessedString: string | null = null;
    private femaleVoice = 'en/en+f3';
    private maleVoice = 'en/en+m3';
    public audioVolume: number = 1;
    ...
    public async speak(text: string, name: string, player: any) {
        ...
        if (this.isPlaying && this.audioQueue.some(item => item.text === text)) {
            return;
        }
...
export class ElevenLabsTextToSpeech {
    ...
    private femaleVoiceId: string;
    private maleVoiceId: string;
    private audioQueue: string[] = [];
    private isPlaying = false;
    private lastProcessedString: string | null = null;
    public audioVolume: number = 0.7;
    ...
    public async speak(text: string, name: string, player: any): Promise<void> {
        ...
        let genderVoiceId = this.maleVoiceId;
        const jsonData = await loadGenderData(femaleNpcs, genderCache);
        if (stringExistsInJson(name.toUpperCase(), jsonData)) genderVoiceId = this.femaleVoiceId;
        if (name === player.self && player.selfFemale) genderVoiceId = this.femaleVoiceId;
        if (name == null || name == "") return
        try {
            const response = await this.textToSpeech(text, genderVoiceId);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const audioContent = await response.blob();
            const audioSrc = URL.createObjectURL(audioContent);
            this.enqueueAudio(audioSrc);
        } catch (error) {
